...
SNAP_DIR=angular-productcat-snapshots
...
do
  ...
  if [[ $i = 0 ]]; then
    ...
    rm -rf $SNAP_DIR/step-$i/app/products
  fi

  if [[ $i = 5 ]]; then
    cp -r app/img $SNAP_DIR/step-0/app/
    cp -r app/products $SNAP_DIR/step-0/app/
  fi
done
...
zip -r angular-productcat-`date +"%y%m%d_%H%M"` $SNAP_DIR
