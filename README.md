# AngularJS product Catalog Tutorial Application

## Overview

This application takes the developer through the process of building a web-application using
AngularJS. The application is loosely based on the **Google product Gallery**, which no longer exists.
Here is a historical reference: [Google product Gallery on WayBack][google-product-gallery]
...
- Add a static list with two products.

### step-2 _AngularJS Templates_

- Convert the static product list to dynamic by:
  - Creating a `productListController` controller.
...
- Add a simple unit test for the `productListController` controller to show how to write tests and
  run them using Karma.

### step-3 _Components_

- Introduce components.
- Combine the controller and the template into a reusable, isolated `productList` component.
- Refactor the application and tests to use the `productList` component.
...
- Add a search box to demonstrate:
  ...
  - How `ngRepeat` automatically shrinks and grows the number of products in the view.
...
- Add an `age` property to the product model.
- Add a drop-down menu to control the product list order.
...
- Replace the in-memory dataset with data loaded from the server (in the form of a static
  'product.json' file to keep the tutorial backend agnostic):
...
- Add a product image and links to product pages.
- Add an end-to-end test that verifies the product links.
...
- Implement fetching data for the selected product and rendering to the view:
  - Use `$http` in `productDetailController` to fetch the product details from a JSON file.
  - Create the template for the detail view.
- Add CSS styles to make the product detail page look "pretty-ish".

### step-11 _Custom Filters_

- Implement a custom `checkmark` filter.
- Update the `productDetail` template to use the `checkmark` filter.
- Add a unit test for the `checkmark` filter.

### step-12 _Event Handlers_

- Make the thumbnail images in the product detail view clickable:
  - Introduce a `mainImageUrl` property on `productDetailController`.
...
- Create a custom `product` service that represents the RESTful client.
- Use a custom Jasmine equality tester in unit tests to ignore irrelevant properties.

### step-14 _Animations_

- Add animations to the application:
  - Animate changes to the product list, adding, removing and reordering products with `ngRepeat`.
  - Animate view transitions with `ngView`.
  - Animate changes to the main product image in the product detail view.
- Showcase three different kinds of animations:
  - CSS transition animations.
  - CSS keyframe animations.
  - JavaScript-based animations.

## Development with `angular-productcat`
...
app/                     --> all the source code of the app (along with unit tests)
  ...
  core/                  --> all the source code of the core module (stuff used throughout the app)
    ...
    product/...            --> files for the `core.product` submodule, including JS source code, specs
    core.module.js       --> the core module
  img/...                --> image files
  product-detail/...       --> files for the `productDetail` module, including JS source code, HTML templates, specs
  product-list/...         --> files for the `productList` module, including JS source code, HTML templates, specs
  products/...             --> static JSON files with product data (used to fake a backend API)
...
[google-product-gallery]: http://web.archive.org/web/20131215082038/http://www.android.com/devices
